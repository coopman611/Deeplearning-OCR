...
titleImage=ImageTk.PhotoImage(Image.open(title_path))
...
def upload():
    ...
    def brightnessUpdate():
        ...
        enhanced_img.save("tempImg.jpg")

        global img2
        img2=ImageTk.PhotoImage(Image.open("tempImg.jpg"))
    ...
    def uploadImage():
        ...
        imageName = os.path.basename("tempImg.jpg")
        imageName = os.path.splitext(imageName)
        
        shutil.copy("tempImg.jpg", dst_path)
        ...
        title_img.configure(image=new_image)
        panel.image=new_image




    upload =Button(imageWin, text="Upload Image", width=10, command=uploadImage)
    upload.grid(row=3, column=0, sticky=W)
    updateBrightness =Button(imageWin, text="Update Image", width=15, command=brightnessUpdate)
    updateBrightness.grid(row=6, column=0, sticky=W, pady=5)
    ...
    resized_image= img2.resize((300,205), Image.ANTIALIAS)
...
title_img.grid(row=4, column=0, pady=5)
